...
class TreeLSTM(nn.Module):
    ...
    def forward(self, g: dgl.DGLGraph, x: torch.Tensor):
        # 添加调试信息
        # print(f"TreeLSTM Debug - g.num_nodes(): {g.num_nodes()}")
        # print(f"TreeLSTM Debug - x.shape: {x.shape}")

        # remove self-loop & reverse on CPU to avoid GPU kernel issues, then move back
        dev = x.device
        try:
            g_cpu = g.to('cpu')
        except Exception:
            g_cpu = g
        g_cpu = dgl.reverse(dgl.remove_self_loop(g_cpu))
        g = g_cpu.to(dev) if str(dev).startswith('cuda') else g_cpu
