...
def analyze_dataset_statistics():
    ...
    with mltk.Experiment(ExpConfig) as exp:
        ...
        span_total = 0
        anomaly_count = 0
        node_count_counter = Counter()
        depth_counter = Counter()

        def _update_stats_from_graph(trace_graph):
            nonlocal trace_count, span_total, anomaly_count, node_count_counter, depth_counter

            trace_count += 1
            node_count = getattr(trace_graph, "node_count", None)
            max_depth = getattr(trace_graph, "max_depth", None)
            anomaly = getattr(trace_graph, "anomaly", 0)

            # 节点总数
            if node_count is not None:
                span_total += int(node_count)
                node_count_counter[int(node_count)] += 1

            # 最大深度
            if max_depth is not None:
                depth_counter[int(max_depth)] += 1
        ...
        # 训练集
        train_dataset = TrainDataset(config, valid=False)
        def train_graph_iter():
            for i in range(len(train_dataset)):
                graph_bytes = train_dataset.train_db.bytes_db.get(i)
                yield TraceGraph.from_bytes(graph_bytes)
        for g in train_graph_iter():
            _update_stats_from_graph(g)
        ...
        try:
            ...
            def test_graph_iter():
                for i in range(len(test_dataset)):
                    ...
                    yield TraceGraph.from_bytes(graph_bytes)
            for g in test_graph_iter():
                _update_stats_from_graph(g)
        except Exception as e:
            print(f"无法加载测试数据集进行统计: {e}")

        # 统一输出整体统计结果
        print("\n--- 整体数据集统计（训练+验证+测试） ---")
        print(f"trace 总数: {trace_count}")
        print(f"span（节点）总数: {span_total}")
        if trace_count > 0:
            print(f"异常 trace 总数: {anomaly_count}")
            print(f"异常率: {anomaly_count / trace_count * 100:.2f}%")
        else:
            print("异常 trace 总数: N/A")
            print("异常率: N/A")

        if node_count_counter:
            print("\n节点总数分布 (node_count -> 样本数):")
            for k in sorted(node_count_counter.keys()):
                print(f"  {k:>4} -> {node_count_counter[k]}")
        else:
            print("\n节点总数分布: N/A")

        if depth_counter:
            print("\n最大深度分布 (max_depth -> 样本数):")
            for k in sorted(depth_counter.keys()):
                print(f"  {k:>4} -> {depth_counter[k]}")
        else:
            print("\n最大深度分布: N/A")
...
def main():
    ...
    print("5. 数据集详细统计（trace数、span数、异常率、节点数/深度分布）")
