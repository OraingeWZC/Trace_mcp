...
__all__ = ['analyze_anomaly_nll', 'analyze_anomaly_scores']

def mad_threshold(data, k=3):
    median = np.median(data)
    mad = np.median(np.abs(data - median))
    constant = 1.4826
    lower = median - k * constant * mad
    upper = median + k * constant * mad
    return lower, upper, median, mad


def analyze_anomaly_scores(
    score_list: np.ndarray,     # 改成通用异常分数
    label_list: np.ndarray,     # 0=normal, 1=结构异常, 2=时延异常
    up_sample_normal: int = 1,
    threshold: float = None,    # 如果 None，则用 μ+3σ
    method: str = None,
    dataset: str = None,
    ...
    if up_sample_normal and up_sample_normal > 1:
        normal_scores = score_list[label_list == 0]
        normal_labels = label_list[label_list == 0]
        score_list = np.concatenate(
            [normal_scores] * (up_sample_normal - 1) + [score_list],
            axis=0
        )
        label_list = np.concatenate(
            [normal_labels] * (up_sample_normal - 1) + [label_list],
            axis=0
        )
    ...
    # ====== 各类均值 ======
    result_dict['score_normal'] = float(np.mean(score_list[label_list == 0]))
    # result_dict['score_structure'] = float(np.mean(score_list[label_list == 1]))
    # result_dict['score_latency'] = float(np.mean(score_list[label_list == 2]))

    # ====== AUC ======
    result_dict['auc'] = float(auc_score(score_list, is_anomaly_list))

    # ====== 最优F1（遍历阈值找最优） ======
    F_best = log_error(best_fscore, default_value=(math.nan, math.nan))

    def best_fscore_for_label(label):
        not_label = 2 if label == 1 else 1
        mask = label_list != not_label
        return F_best(score_list[mask], label_list[mask] != 0)

    best_fscore_total, best_threshold, best_pr_total, best_rc_total = F_best(score_list, is_anomaly_list)
    best_fscore_structure, _, best_pr_structure, best_rc_structure = best_fscore_for_label(1)
    best_fscore_latency, _, best_pr_latency, best_rc_latency = best_fscore_for_label(2)
    result_dict.update({
        'best_threshold': float(best_threshold),
        'best_fscore': float(best_fscore_total),
        # 'best_fscore_structure': float(best_fscore_structure),
        # 'best_fscore_latency': float(best_fscore_latency),
        'best_pr': float(best_pr_total),
        'best_rc': float(best_rc_total),
        # 'best_pr_structure': float(best_pr_structure),
        # 'best_rc_structure': float(best_rc_structure),
    ...
    if threshold is None:
        _, threshold, median, mad = mad_threshold(score_list)
    result_dict['threshold'] = float(threshold)
    result_dict['median'] = float(median)
    result_dict['mad'] = float(mad)

    # ====== F1 at 阈值 ======
    F = log_error(f1_score, default_value=math.nan)
    P = log_error(precision_score, default_value=math.nan)
    R = log_error(recall_score, default_value=math.nan)

    def fscore_for_label(label):
        not_label = 2 if label == 1 else 1
        mask = label_list != not_label
        return F(label_list[mask] != 0, score_list[mask] > threshold)

    result_dict.update({
        'fscore': float(F(is_anomaly_list, score_list > threshold)),
        'precision': float(P(is_anomaly_list, score_list > threshold)),
        'recall': float(R(is_anomaly_list, score_list > threshold)),
        # 'fscore_structure': float(fscore_for_label(1)),
    ...
    if save_dict and method and dataset:
        ...
        file_path = f'paper-data/{save_filename}'
        if os.path.exists(file_path):
            ...
            if not df[(df['dataset'] == dataset) & (df['method'] == method)].empty:
                ...
                df = pd.concat([df, pd.DataFrame([result_to_save])], ignore_index=True)
        else:
