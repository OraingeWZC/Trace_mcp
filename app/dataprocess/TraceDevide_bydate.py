...
# ---------- 文件名日期提取 ----------
def _extract_date_from_name(p: pl.Path) -> str:
    """
    从文件名中提取日期（优先 YYYY-MM-DD；其次 YYYYMMDD）。失败则返回空串。
    """
    name = p.name
...
def split_one_csv(csv_path: pl.Path, gt_df: pd.DataFrame, out_root: pl.Path, global_wrote_header):
    ...
    for chunk in pd.read_csv(csv_path, chunksize=2_000_000):
        ...
        # 计算当前 CSV 的日期（优先取文件名；否则用首个分片的 StartTimeMs 推断 UTC 日期）
        if "_date_str" not in locals() or not _date_str:
            _date_str = _extract_date_from_name(csv_path)
        if not _date_str:
            try:
                # 允许列名为 StartTimeMs 或 Normalized_StartTime（毫秒）
                if "StartTimeMs" in part.columns:
                    ms_val = pd.to_numeric(part["StartTimeMs"], errors="coerce").dropna().astype(float)
                elif "Normalized_StartTime" in part.columns:
                    ms_val = pd.to_numeric(part["Normalized_StartTime"], errors="coerce").dropna().astype(float)
                else:
                    ms_val = pd.Series([], dtype=float)
                if len(ms_val) > 0:
                    ts = float(ms_val.iloc[0]) / 1000.0
                    _date_str = datetime.utcfromtimestamp(ts).strftime("%Y-%m-%d")
                else:
                    _date_str = "unknown_date"
            except Exception:
                _date_str = "unknown_date"
        ...
        for sub, grp in part.groupby("_sub", sort=False):
            if grp.empty:
                continue
            # 目标文件：out_root/<sub>/<YYYY-MM-DD>_spans.csv
            path = out_root / sub / f"{_date_str}_spans.csv"
            first_time = not path.exists()
            grp.drop(columns=["_sub"]).to_csv(
                path, mode="a",
                index=False, header=first_time,
...
def main():
    ...
    ap.add_argument("--csv-dir", default='../../dataset/aiops25/processed/phaseone', type=pl.Path, help="目录（不递归）")
    ap.add_argument("--groundtruth", default='E:\ZJU\AIOps\Projects\TraDNN\TraDiag/trace_svnd_all\dataset\Data\groundtruth.csv', type=pl.Path)
    ap.add_argument("--out-root", default="../../dataset/SplitTrace0609", type=pl.Path)
