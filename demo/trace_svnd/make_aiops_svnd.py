...
def build_records(df: pd.DataFrame, cols, api_vocab, status_vocab, node_vocab,
                  ...
                  min_trace_size=2,
                  infra_index=None,
                  infra_metrics: Optional[List[str]] = None,
                  infra_W: int = 3,
                  infra_per_metric_dims: int = 4,
                  desc: str = "Building") -> List[dict]:  # <--- 新增 desc 参数
    t_col, s_col, p_col = cols["trace"], cols["span"], cols["parent"]
    ...
    # infra 配置
    use_infra = infra_index is not None
    metrics = list(infra_metrics) if infra_metrics is not None else list(DEFAULT_METRICS)
    infra_dim = int(infra_per_metric_dims) * max(1, len(metrics))
    zero_host_state: List[float] = [0.0] * infra_dim if infra_dim > 0 else []

    records = []
    
    # --- [修改点] 计算总数并添加进度条 ---
    total_traces = df[t_col].nunique()
    # 使用 tqdm 包裹 groupby 对象
    iterator = tqdm(df.groupby(t_col, sort=False), total=total_traces, desc=desc, unit="trace")
    
    for tid, g in iterator:
        g = g.dropna(subset=[s_col, st_col, et_col]).copy()
        if len(g) < min_trace_size:
            continue
        g = g.reset_index(drop=True)

        # per-trace time anchor
        if use_infra:
            try:
                t0_val = pd.to_numeric(g[st_col], errors="coerce").min()
                t0_ms = int(float(t0_val))
        ...
        # host_state cache
        host_state_cache: Dict[str, List[float]] = {}
        nodes = []
        for i in range(len(g)):
            node_name = str(g.loc[i, "_node"])
            host_state_vec: Optional[List[float]] = None
            if use_infra and infra_dim > 0 and t0_min_ms is not None:
                if node_name not in host_state_cache:
                    vec = host_state_vector(
                        node_name, infra_index, t0_min_ms,
                        metrics=metrics, W=int(infra_W),
                        per_metric_dims=int(infra_per_metric_dims),
                    )
                    host_state_cache[node_name] = vec.astype(np.float32).tolist() if vec is not None else list(zero_host_state)
                host_state_vec = host_state_cache.get(node_name, list(zero_host_state))
            ...
            if host_state_vec is not None and len(host_state_vec) == infra_dim and infra_dim > 0:
                node_rec["host_state"] = host_state_vec
...
def parse_args():
    ...
    ap.add_argument("--normal",  default=r"E:\ZJU\AIOps\Projects\TraDNN\dataset\Data\Normal.csv")
    ap.add_argument("--service", default=r"E:\ZJU\AIOps\Projects\TraDNN\dataset\SplitTrace\service\merged.csv")
    ap.add_argument("--node",    default=r"E:\ZJU\AIOps\Projects\TraDNN\dataset\SplitTrace\node\merged.csv")
    ap.add_argument("--outdir", default="dataset/aiops_svnd_1209")
...
def main():
    ...
    # 基础设施指标：尝试从 outdir 附近加载 merged_all_infra.csv
    try:
        infra_index = load_host_infra_index(args.outdir)
    except Exception:
        infra_index = None
    if infra_index is None:
        print("[3.5/5] no merged_all_infra.csv found or failed to load; skip host_state features.")
    else:
        print(f"[3.5/5] loaded infra index for {len(infra_index)} hosts from merged_all_infra.csv")

    print("[4/5] build jsonl records ...")
    api_vocab, status_vocab, node_vocab = {}, {}, {}
    
    rec_n = build_records(
        df_n, cols, api_vocab, status_vocab, node_vocab,
        fixed_c3=0, fault_type_col="fault_type", min_trace_size=args.min_trace_size,
        infra_index=infra_index, infra_metrics=DEFAULT_METRICS,
        infra_W=int(round(args.win_minutes)), infra_per_metric_dims=4,
        desc="[1/3 Normal]"  # <--- 加这里
    )
    
    rec_s = build_records(
        df_s, cols, api_vocab, status_vocab, node_vocab,
        fixed_c3=1, fault_type_col="fault_type", min_trace_size=args.min_trace_size,
        infra_index=infra_index, infra_metrics=DEFAULT_METRICS,
        infra_W=int(round(args.win_minutes)), infra_per_metric_dims=4,
        desc="[2/3 Service]" # <--- 加这里
    )
    
    rec_f = build_records(
        df_f, cols, api_vocab, status_vocab, node_vocab,
        fixed_c3=2, fault_type_col="fault_type", min_trace_size=args.min_trace_size,
        infra_index=infra_index, infra_metrics=DEFAULT_METRICS,
        infra_W=int(round(args.win_minutes)), infra_per_metric_dims=4,
        desc="[3/3 Node   ]" # <--- 加这里
    )
